...
object TagsContext4 {
  def main(args: Array[String]): Unit = {
    ...
    val baseRDD = df.filter(TagUtils.OneUserId)
      // 接下来所有的标签都在内部实现
      .map(row => {
      val list = TagUtils.getAllUserId(row)
      (list, row)
    })
    //todo 图计算
    //点
    val vertiesRDD: RDD[(Long, List[(String, Int)])] = baseRDD.flatMap(tp => {
      ...
      tp._1.map(tId => {
        //保证一个id携带标签,取第一个携带标签
        if (tp._1.head.equals(tId)) {
          (tId.hashCode.toLong, VD)
        } else {
          (tId.hashCode.toLong, List.empty)
        }
      })
    })
    //边
    ...
    vertices.join(vertiesRDD).map((_._2)).reduceByKey((list1,list2)=>{
